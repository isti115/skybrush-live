...
export class WHEPClient {
  ...
  start() {
    ...
    fetch(new URL('whep', this.href) + winlocsearch, {
      method: 'OPTIONS',
    })
  ...
  sendLocalCandidates(candidates) {
    fetch(this.sessionUrl + winlocsearch, {
      method: 'PATCH',
      headers: {
 ...
 *
 * @param {string} [url=winlocsearch] - The URL to parse the query string from.
...
const parseQueryString = (url) => {
  const queryString = (url || winlocsearch).split('?')[1];
 ...
 */
export const setVideoAttributes = (video) => {
 ...
 * @returns
 */
export const initVideoElement = (callback, container) => {
  return () => {
...
window.addEventListener(
  'DOMContentLoaded',
  initVideoElement((video) => new WHEPClient(video), document.body)
);
